...
import EyeslashSvg from "./eye_slash";
...
export default function Signup() {
    ...
    const onSubmit = (event) => {
        ...
        fetch('your_backend_url', { // Replace 'your_backend_url' with the actual URL
            method: "POST",
            body: formData
        }).then(resp => resp.json())
            .then(data => {
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
            ...
            <form onSubmit={onSubmit} className="mx-auto mb-0 mt-8 max-w-md space-y-4">
                ...
                <div>
                    ...
                    {error && <p className="text-red-500 text-sm mt-1">{pwdNotMatch}</p>}
                </div>

                <div className="flex items-center justify-between">
                    <p className="text-sm text-gray-500">
                        Already have an account?
                        <Link to="/dashboard" className="text-blue-500"> Sign in</Link>
                    </p>
                    <button
                        type="submit"
                        className={btnStyle}
