...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchStudentCount = async () => {
      try {
        const response = await fetch('http://localhost:5173/student_count'); // Update the endpoint if needed
        if (response.ok) {
  ...
  useEffect(() => {
    const fetcchCourseCount = async () => {
      try {
        const response = await fetch('http://localhost:5173/course_count')
      }
    }
  })
